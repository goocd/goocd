...
// NOTE: code borrowed and modified from TinyGo and then hacked to pieces
...
func writeGo(outdir string, device *Device, interruptSystem string) error {
	pkgDir := filepath.Join(outdir, device.Metadata.NameLower)
	os.Mkdir(pkgDir, 0755)
	outf, err := os.Create(filepath.Join(pkgDir, device.Metadata.NameLower+".go"))
	if err != nil {
		return err
	}
	defer outf.Close()
...
package {{.device.Metadata.NameLower}}
...
{{- range .device.Peripherals}}
	...
	// {{.}}
		{{- end}}
	{{- end}}
	{{.Name}}_Addr = 0x{{printf "%x" .BaseAddress}}
	{{- "\n"}}
{{- end}}
)
...
`))
	...
	for _, peripheral := range device.Peripherals {
		...
		for _, register := range peripheral.Registers {
			...
			case 8:
				regType = "uint64" //"volatile.Register64"
			case 4:
				regType = "uint32" // "volatile.Register32"
			case 2:
				regType = "uint16" // "volatile.Register16"
			case 1:
				regType = "uint8" // "volatile.Register8"
			default:
				regType = "uint32" // "volatile.Register32"
			}
			...
			regoffset := register.Address - peripheral.BaseAddress
			fmt.Fprintf(w, "\t%s_%s_Offset = 0x%X // %d\n", peripheral.GroupName, register.Name, regoffset, regoffset)
			if register.ElementSize > 0 {
				fmt.Fprintf(w, "\t%s_%s_Size = %d\n", peripheral.GroupName, register.Name, register.ElementSize)
			}
...
func main() {
	...
	interruptSystem := flag.String("interrupts", "software", "interrupt system in use (software, hardware)")
	flag.Parse()
	if flag.NArg() != 2 {
